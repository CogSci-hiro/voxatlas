...
from voxatlas.phonology.rhythm_utils import compute_delta_v
from voxatlas.registry.feature_registry import registry


class RhythmDeltaVExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.rhythm.delta_v`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.rhythm.delta_v`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``ipu`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    2. Metric computation
       Vowel-duration variability is :math:`\Delta V = \mathrm{sd}(d^{(v)})`.
    ...
    ...     [{"ipu_id": 1, "type": "v", "duration": 0.5}, {"ipu_id": 1, "type": "v", "duration": 1.5}]
    ... )
    >>> store = FeatureStore()
    >>> store.add(
    ...
    ... )
    >>> out = RhythmDeltaVExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> round(float(out.values.loc[1]), 1)
    0.5
    """
    name = "phonology.rhythm.delta_v"
    input_units = "phoneme"
    output_units = "ipu"
