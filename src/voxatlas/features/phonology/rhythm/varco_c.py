...
class RhythmVarcoCExtractor(BaseExtractor):
    ...
    ...     [{"ipu_id": 1, "type": "c", "duration": 1.0}, {"ipu_id": 1, "type": "c", "duration": 2.0}]
    ... )
    >>> store = FeatureStore()
    >>> store.add(
    ...
    ... )
    >>> out = RhythmVarcoCExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    ...
    def compute(self, feature_input, params):
        ...
        >>> result = RhythmVarcoCExtractor().compute(feature_input, {})
        >>> result.unit
        'ipu'
        """
