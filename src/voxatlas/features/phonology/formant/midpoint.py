...
from voxatlas.phonology.formant_utils import compute_vowel_midpoint
from voxatlas.registry.feature_registry import registry


class FormantMidpointExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.midpoint`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.formant.midpoint`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    3. Metric computation
       For each vowel segment, the midpoint sample is selected and the corresponding formant vector :math:`(F_1, F_2, F_3)` is returned.
    ...
    >>> out = FormantMidpointExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> out.values.shape[0]
    1
    """
    name = "phonology.formant.midpoint"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantMidpointExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'phoneme'
        """
        ...
        return TableFeatureOutput(
            feature=self.name,
            unit="phoneme",
            values=compute_vowel_midpoint(tracks),
        )


registry.register(FormantMidpointExtractor)
