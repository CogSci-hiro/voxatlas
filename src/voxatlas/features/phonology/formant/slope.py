...
class FormantSlopeExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.slope`` feature within the VoxAtlas pipeline.
    ...
    3. Metric computation
       The extractor estimates linear trajectory slope for each formant, :math:`\hat\beta_m`, by regressing formant value against normalized time.
    ...
    >>> out = FormantSlopeExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> "F1_slope" in out.values.columns
    True
    """
    name = "phonology.formant.slope"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantSlopeExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'phoneme'
        """
