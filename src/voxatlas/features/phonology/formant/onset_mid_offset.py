...
from voxatlas.phonology.formant_utils import compute_onset_mid_offset
from voxatlas.registry.feature_registry import registry


class FormantOnsetMidOffsetExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.onset_mid_offset`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.formant.onset_mid_offset`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    3. Metric computation
       The extractor samples formants at normalized onset, midpoint, and offset positions to provide a compact dynamic descriptor.
    ...
    >>> from voxatlas.features.feature_output import TableFeatureOutput
    >>> from voxatlas.features.phonology.formant.onset_mid_offset import FormantOnsetMidOffsetExtractor
    ...
    >>> out = FormantOnsetMidOffsetExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> any(col.endswith("_onset") for col in out.values.columns)
    True
    """
    name = "phonology.formant.onset_mid_offset"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantOnsetMidOffsetExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'phoneme'
        """
        ...
        return TableFeatureOutput(
            feature=self.name,
            unit="phoneme",
            values=compute_onset_mid_offset(tracks),
        )


registry.register(FormantOnsetMidOffsetExtractor)
