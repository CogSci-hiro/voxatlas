...
from voxatlas.phonology.formant_utils import compute_vowel_variance
from voxatlas.registry.feature_registry import registry


class FormantVarianceExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.variance`` feature within the VoxAtlas pipeline.
    ...
    3. Metric computation
       The extractor computes within-unit formant variance, :math:`\mathrm{Var}(F_m) = \frac{1}{N}\sum_i(F_{m,i}-\bar F_m)^2`.
    ...
    >>> out = FormantVarianceExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> out.values.columns.tolist()[:2]
    ['phoneme_id', 'label']
    """
    name = "phonology.formant.variance"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantVarianceExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'phoneme'
        """
        ...
        return TableFeatureOutput(
            feature=self.name,
            unit="phoneme",
            values=compute_vowel_variance(tracks),
        )


registry.register(FormantVarianceExtractor)
