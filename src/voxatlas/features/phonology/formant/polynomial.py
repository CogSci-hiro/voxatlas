...
from voxatlas.phonology.formant_utils import compute_polynomial_coefficients
from voxatlas.registry.feature_registry import registry


class FormantPolynomialExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.polynomial`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.formant.polynomial`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    3. Metric computation
       The extractor fits low-order polynomial coefficients to the formant trajectory so that smooth vowel dynamics can be modeled compactly.
    ...
    >>> from voxatlas.features.feature_output import TableFeatureOutput
    >>> from voxatlas.features.phonology.formant.polynomial import FormantPolynomialExtractor
    ...
    >>> params = FormantPolynomialExtractor.default_config.copy()
    >>> params["degree"] = 2
    >>> out = FormantPolynomialExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), params)
    >>> "F1_c0" in out.values.columns
    True
    """
    name = "phonology.formant.polynomial"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantPolynomialExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'phoneme'
        """
        ...
        return TableFeatureOutput(
            feature=self.name,
            unit="phoneme",
            values=compute_polynomial_coefficients(
                tracks,
...
registry.register(FormantPolynomialExtractor)
