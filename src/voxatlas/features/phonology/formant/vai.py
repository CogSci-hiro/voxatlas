...
from voxatlas.phonology.formant_utils import compute_vai
from voxatlas.registry.feature_registry import registry


class FormantVAIExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.vai`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.formant.vai`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``conversation`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    3. Metric computation
       The extractor computes a vowel-articulation index from formant combinations that contrast peripheral and centralized vowel realizations.
    ...
    >>> out = FormantVAIExtractor().compute(FeatureInput(audio=None, units=Units(speaker="A"), context={"feature_store": store}), {})
    >>> out.unit
    'conversation'
    """
    name = "phonology.formant.vai"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("phonology.formant.tracks", TableFeatureOutput(feature="phonology.formant.tracks", unit="frame", values=tracks))
        >>> result = FormantVAIExtractor().compute(FeatureInput(audio=None, units=Units(speaker="A"), context={"feature_store": store}), {})
        ...
        values = pd.Series([compute_vai(tracks)], index=[speaker], dtype="float32")
        return ScalarFeatureOutput(feature=self.name, unit="conversation", values=values)


registry.register(FormantVAIExtractor)
