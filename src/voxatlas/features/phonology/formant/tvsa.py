...
from voxatlas.phonology.formant_utils import compute_tvsa
from voxatlas.registry.feature_registry import registry


class FormantTVSAExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.formant.tvsa`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.formant.tvsa`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``conversation`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    3. Metric computation
       The extractor computes triangular vowel-space area from three anchor vowels in the :math:`F_1 \times F_2` plane.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series([compute_tvsa(tracks)], index=[speaker], dtype="float32")
        return ScalarFeatureOutput(feature=self.name, unit="conversation", values=values)


registry.register(FormantTVSAExtractor)
