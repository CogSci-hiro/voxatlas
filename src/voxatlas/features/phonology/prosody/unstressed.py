...
class ProsodicUnstressedExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.prosody.unstressed`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.prosody.unstressed`` from VoxAtlas structured inputs. It consumes ``syllable`` units and produces values aligned to ``syllable`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(
            (1.0 - stressed.to_numpy(dtype="float32")).astype("float32"),
...
registry.register(ProsodicUnstressedExtractor)
