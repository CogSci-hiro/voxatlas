...
class ProsodicPositionInWordExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.prosody.position_in_word`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.prosody.position_in_word`` from VoxAtlas structured inputs. It consumes ``syllable`` units and produces values aligned to ``syllable`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> from voxatlas.features.phonology.prosody.position_word import ProsodicPositionInWordExtractor
    ...
    >>> units = Units(syllables=syllables, words=words)
    >>> out = ProsodicPositionInWordExtractor().compute(FeatureInput(audio=None, units=units, context={}), {})
    >>> int(out.values.loc[10]), int(out.values.loc[11])
    (1, 2)
    """
    name = "phonology.prosody.position_in_word"
    ...
    def compute(self, feature_input, params):
        ...
        >>> words = pd.DataFrame({"id": [1], "start": [0.0], "end": [1.0]})
        >>> units = Units(syllables=syllables, words=words)
        >>> result = ProsodicPositionInWordExtractor().compute(FeatureInput(audio=None, units=units, context={}), {})
        >>> result.unit
        'syllable'
        """
        if feature_input.units is None:
            raise ValueError(f"{self.name} requires syllable and word units")

        units = feature_input.units
        positions = compute_word_positions(
            syllables=units.get("syllable"),
            words=units.get("word"),
        )
        values = pd.Series(
            positions["position_in_word"].array,
...
registry.register(ProsodicPositionInWordExtractor)
