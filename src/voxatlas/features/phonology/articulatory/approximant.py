...
class ArticulatoryApproximantExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.articulatory.approximant`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.articulatory.approximant`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    --------
    >>> import pandas as pd
    >>> from voxatlas.features.phonology.articulatory.approximant import ArticulatoryApproximantExtractor
    >>> from voxatlas.features.feature_input import FeatureInput
    >>> from voxatlas.features.feature_output import TableFeatureOutput
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1, 2], "approximant": [1, 0]})
    >>> store = FeatureStore()
    >>> store.add(
    ...
    >>> out = ArticulatoryApproximantExtractor().compute(feature_input, {})
    >>> float(out.values.loc[1])
    1.0
    >>> float(out.values.loc[2])
    0.0
    """
    name = "phonology.articulatory.approximant"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        >>> from voxatlas.features.feature_output import TableFeatureOutput
        >>> from voxatlas.pipeline.feature_store import FeatureStore
        >>> table = pd.DataFrame({"id": [1], "approximant": [1]})
        >>> store = FeatureStore()
        >>> store.add(
        ...     "phonology.articulatory.features",
        ...     TableFeatureOutput(feature="phonology.articulatory.features", unit="phoneme", values=table),
        ... )
        >>> feature_input = FeatureInput(audio=None, units=None, context={"feature_store": store})
        >>> result = ArticulatoryApproximantExtractor().compute(feature_input, {})
        >>> result.unit
        'phoneme'
        """
        ...
        values = pd.Series(table["approximant"].astype("float32").values, index=table["id"])
        return ScalarFeatureOutput(feature=self.name, unit="phoneme", values=values)


registry.register(ArticulatoryApproximantExtractor)
