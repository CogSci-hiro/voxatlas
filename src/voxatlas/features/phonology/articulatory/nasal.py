...
class ArticulatoryNasalExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.articulatory.nasal`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.articulatory.nasal`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> from voxatlas.features.phonology.articulatory.nasal import ArticulatoryNasalExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "nasal": [1]})
    >>> store = FeatureStore()
    >>> store.add(
    ...
    >>> out = ArticulatoryNasalExtractor().compute(feature_input, {})
    >>> float(out.values.loc[1])
    1.0
    """
    name = "phonology.articulatory.nasal"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        ... )
        >>> feature_input = FeatureInput(audio=None, units=None, context={"feature_store": store})
        >>> result = ArticulatoryNasalExtractor().compute(feature_input, {})
        >>> result.unit
        'phoneme'
        """
        ...
        values = pd.Series(table["nasal"].astype("float32").values, index=table["id"])
        return ScalarFeatureOutput(feature=self.name, unit="phoneme", values=values)


registry.register(ArticulatoryNasalExtractor)
