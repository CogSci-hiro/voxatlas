...
class ArticulatoryFricativeExtractor(BaseExtractor):
    r"""
    Extract the ``phonology.articulatory.fricative`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``phonology.articulatory.fricative`` from VoxAtlas structured inputs. It consumes ``phoneme`` units and produces values aligned to ``phoneme`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(table["fricative"].astype("float32").values, index=table["id"])
        return ScalarFeatureOutput(feature=self.name, unit="phoneme", values=values)


registry.register(ArticulatoryFricativeExtractor)
