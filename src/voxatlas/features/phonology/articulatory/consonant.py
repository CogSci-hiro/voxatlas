...
class ArticulatoryConsonantExtractor(BaseExtractor):
    r"""
    ...
    --------
        ...
        extractor = ArticulatoryConsonantExtractor()
        feature_input = FeatureInput(audio=audio, units=units, context={})
        output = extractor.compute(feature_input, {})
        print(output)
    """
    name = "phonology.articulatory.consonant"
    input_units = "phoneme"
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(table["consonant"].astype("float32").values, index=table["id"])
        return ScalarFeatureOutput(feature=self.name, unit="phoneme", values=values)


registry.register(ArticulatoryConsonantExtractor)
