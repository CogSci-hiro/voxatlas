...
class POSExtractor(BaseExtractor):
    r"""
    Extract the ``lexical.properties.pos`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``lexical.properties.pos`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> table = pd.DataFrame({"id": [1], "pos": ["NOUN"]})
    >>> store = FeatureStore()
    >>> store.add("lexical.properties.lookup", TableFeatureOutput(feature="lexical.properties.lookup", unit="token", values=table))
    >>> out = POSExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> out.values.loc[1]
    'NOUN'
    """
    name = "lexical.properties.pos"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("lexical.properties.lookup", TableFeatureOutput(feature="lexical.properties.lookup", unit="token", values=table))
        >>> result = POSExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(table["pos"].values, index=table["id"], dtype="object")
