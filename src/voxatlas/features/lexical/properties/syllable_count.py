...
class SyllableCountExtractor(BaseExtractor):
    r"""
    Extract the ``lexical.properties.syllable_count`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``lexical.properties.syllable_count`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(
            table["syllable_count"].astype("float32").values,
            index=table["id"],
            dtype="float32",
        )
...
registry.register(SyllableCountExtractor)
