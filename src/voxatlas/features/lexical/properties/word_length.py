...
class WordLengthExtractor(BaseExtractor):
    r"""
    Extract the ``lexical.properties.word_length`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``lexical.properties.word_length`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> table = pd.DataFrame({"id": [1], "word_length": [5]})
    >>> store = FeatureStore()
    >>> store.add("lexical.properties.features", TableFeatureOutput(feature="lexical.properties.features", unit="token", values=table))
    >>> out = WordLengthExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> float(out.values.loc[1])
    5.0
    """
    name = "lexical.properties.word_length"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("lexical.properties.features", TableFeatureOutput(feature="lexical.properties.features", unit="token", values=table))
        >>> result = WordLengthExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        table = feature_input.context["feature_store"].get("lexical.properties.features").values
        values = pd.Series(
            table["word_length"].astype("float32").values,
            index=table["id"],
            dtype="float32",
        )
...
registry.register(WordLengthExtractor)
