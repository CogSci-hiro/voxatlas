...
SUBORDINATE_CLAUSE_LABELS = {"ccomp", "xcomp", "advcl", "acl"}


class SyntaxSubordinateClauseExtractor(BaseExtractor):
    r"""
    Extract the ``syntax.clause_structure.subordinate_clause`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``syntax.clause_structure.subordinate_clause`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
...
registry.register(SyntaxSubordinateClauseExtractor)
