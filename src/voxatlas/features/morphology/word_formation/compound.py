...
class CompoundExtractor(BaseExtractor):
    ...
    >>> from voxatlas.features.morphology.word_formation.compound import CompoundExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "Compound": [1.0]})
    >>> store = FeatureStore()
    ...
    >>> out = CompoundExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> float(out.values.loc[1])
    1.0
    """
    name = "morphology.word_formation.compound"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.word_formation.features", TableFeatureOutput(feature="morphology.word_formation.features", unit="token", values=table))
        >>> result = CompoundExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(
            table["Compound"].astype("float32").values,
            index=table["id"],
        )
...
registry.register(CompoundExtractor)
