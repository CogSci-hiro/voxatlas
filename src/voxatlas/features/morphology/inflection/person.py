...
class InflectionPersonExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.inflection.person`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``morphology.inflection.person`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> from voxatlas.features.morphology.inflection.person import InflectionPersonExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "Person": [3]})
    >>> store = FeatureStore()
    ...
    >>> out = InflectionPersonExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> out.values.loc[1]
    3
    """
    name = "morphology.inflection.person"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.inflection.features", TableFeatureOutput(feature="morphology.inflection.features", unit="token", values=table))
        >>> result = InflectionPersonExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(table["Person"].values, index=table["id"], dtype="object")
...
registry.register(InflectionPersonExtractor)
