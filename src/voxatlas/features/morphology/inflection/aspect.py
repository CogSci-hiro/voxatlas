...
class InflectionAspectExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.inflection.aspect`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``morphology.inflection.aspect`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(table["Aspect"].values, index=table["id"], dtype="object")
...
registry.register(InflectionAspectExtractor)
