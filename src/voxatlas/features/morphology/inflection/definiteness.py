...
class InflectionDefinitenessExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.inflection.definiteness`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``morphology.inflection.definiteness`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(table["Definite"].values, index=table["id"], dtype="object")
...
registry.register(InflectionDefinitenessExtractor)
