...
class VerbMorphologyInfinitiveExtractor(BaseExtractor):
    ...
    >>> from voxatlas.features.morphology.verb_morphology.infinitive import VerbMorphologyInfinitiveExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "Infinitive": [1.0]})
    >>> store = FeatureStore()
    ...
    >>> out = VerbMorphologyInfinitiveExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> float(out.values.loc[1])
    1.0
    """
    name = "morphology.verb_morphology.infinitive"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.verb_morphology.features", TableFeatureOutput(feature="morphology.verb_morphology.features", unit="token", values=table))
        >>> result = VerbMorphologyInfinitiveExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(
            table["Infinitive"].astype("float32").values,
            index=table["id"],
        )
...
registry.register(VerbMorphologyInfinitiveExtractor)
