...
class VerbMorphologyFiniteExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.verb_morphology.finite`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``morphology.verb_morphology.finite`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    def compute(self, feature_input, params):
        ...
        values = pd.Series(table["Finite"].astype("float32").values, index=table["id"])
...
registry.register(VerbMorphologyFiniteExtractor)
