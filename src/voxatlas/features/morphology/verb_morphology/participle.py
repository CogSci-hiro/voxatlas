...
class VerbMorphologyParticipleExtractor(BaseExtractor):
    ...
    >>> table = pd.DataFrame({"id": [1], "Participle": [1.0]})
    >>> store = FeatureStore()
    ...
    >>> out = VerbMorphologyParticipleExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.verb_morphology.features", TableFeatureOutput(feature="morphology.verb_morphology.features", unit="token", values=table))
        >>> result = VerbMorphologyParticipleExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
