...
class PrefixPresenceExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.derivation.prefix_presence`` feature within the VoxAtlas pipeline.
    
    This public extractor defines the reusable API for computing ``morphology.derivation.prefix_presence`` from VoxAtlas structured inputs. It consumes ``token`` units and produces values aligned to ``token`` units, making the extractor a stable pipeline node that can be cited independently of the surrounding execution machinery.
    ...
    >>> from voxatlas.features.morphology.derivation.prefix_presence import PrefixPresenceExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "prefix_presence": [1.0]})
    >>> store = FeatureStore()
    ...
    >>> out = PrefixPresenceExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> float(out.values.loc[1])
    1.0
    """
    name = "morphology.derivation.prefix_presence"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.derivation.segmentation", TableFeatureOutput(feature="morphology.derivation.segmentation", unit="token", values=table))
        >>> result = PrefixPresenceExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(
            table["prefix_presence"].astype("float32").values,
            index=table["id"],
            dtype="float32",
        )
...
registry.register(PrefixPresenceExtractor)
