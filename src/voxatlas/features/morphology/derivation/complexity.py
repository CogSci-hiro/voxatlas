...
class MorphologicalComplexityExtractor(BaseExtractor):
    r"""
    Extract the ``morphology.derivation.morphological_complexity`` feature within the VoxAtlas pipeline.
    ...
    >>> from voxatlas.features.morphology.derivation.complexity import MorphologicalComplexityExtractor
    >>> from voxatlas.pipeline.feature_store import FeatureStore
    >>> table = pd.DataFrame({"id": [1], "morphological_complexity": [1.0]})
    >>> store = FeatureStore()
    ...
    >>> out = MorphologicalComplexityExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
    >>> float(out.values.loc[1])
    1.0
    """
    name = "morphology.derivation.morphological_complexity"
    input_units = "token"
    output_units = "token"
    ...
    def compute(self, feature_input, params):
        ...
        >>> store = FeatureStore()
        >>> store.add("morphology.derivation.segmentation", TableFeatureOutput(feature="morphology.derivation.segmentation", unit="token", values=table))
        >>> result = MorphologicalComplexityExtractor().compute(FeatureInput(audio=None, units=None, context={"feature_store": store}), {})
        >>> result.unit
        'token'
        """
        ...
        values = pd.Series(
            table["morphological_complexity"].astype("float32").values,
            index=table["id"],
            dtype="float32",
        )
